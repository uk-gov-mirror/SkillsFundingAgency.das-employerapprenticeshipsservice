namespace SFA.DAS.EmployerAccounts.Api.Orchestrators
{
    ...
    public class AccountsOrchestrator
    {
        ...
        {
            this._logger.Info("Getting all account balances.");

            toDate = toDate ?? DateTime.MaxValue.ToString("yyyyMMddHHmmss");

            var accountsResult = await this._mediator.SendAsync(new GetPagedEmployerAccountsQuery { ToDate = toDate, PageSize = pageSize, PageNumber = pageNumber });
            var transactionResult = await this._mediator.SendAsync(new GetAccountBalancesRequest
            {
            ...
            var accountBalanceHash = this.BuildAccountBalanceHash(transactionResult.Accounts);
        ...
        {
            var hashedAccountId = this._hashingService.HashValue(accountId);
            ...
            var response = await this.GetAccount(hashedAccountId);
            return response;
        }

        public async Task<OrchestratorResponse<AccountDetailViewModel>> GetAccount(string hashedAccountId)
        {
            this._logger.Info($"Getting account {hashedAccountId}");

            var accountResult = await this._mediator.SendAsync(new GetEmployerAccountByHashedIdQuery { HashedAccountId = hashedAccountId });
            ...
            var accountBalanceTask = this.GetBalanceForAccount(accountResult.Account.AccountId);
            var transferBalanceTask = this.GetTransferAllowanceForAccount(accountResult.Account.AccountId);
        ...
        {
            this._logger.Info($"Getting paye scheme {payeSchemeRef} for account {hashedAccountId}");

            var payeSchemeResult = await this._mediator.SendAsync(new GetPayeSchemeByRefQuery { HashedAccountId = hashedAccountId, Ref = payeSchemeRef });
        ...
        public async Task<OrchestratorResponse<ICollection<TeamMemberViewModel>>> GetAccountTeamMembers(long accountId)
        {
            ...
            var response = await this.GetAccountTeamMembers(hashedAccountId);
        ...
        {
            this._logger.Info($"Requesting team members for account {hashedAccountId}");

            var teamMembers = await this._mediator.SendAsync(new GetTeamMembersRequest { HashedAccountId = hashedAccountId });

            var memberViewModels = teamMembers.TeamMembers.Select(x => this._mapper.Map<TeamMemberViewModel>(x)).ToList();
        ...
        {
            this._logger.Info($"Requesting levy declaration for account {hashedAccountId}");

            var levyDeclarations = await this._mediator.SendAsync(new GetLevyDeclarationRequest { HashedAccountId = hashedAccountId });
            ...
            var levyViewModels = levyDeclarations.Declarations.Select(x => this._mapper.Map<LevyDeclarationViewModel>(x)).ToList();
        ...
        {
            this._logger.Info($"Requesting levy declaration for account {hashedAccountId}, year {payrollYear} and month {payrollMonth}");

            var levyDeclarations = await this._mediator.SendAsync(new GetLevyDeclarationsByAccountAndPeriodRequest { HashedAccountId = hashedAccountId, PayrollYear = payrollYear, PayrollMonth = payrollMonth });
            if (levyDeclarations.Declarations == null)
            {
                return new OrchestratorResponse<AccountResourceList<LevyDeclarationViewModel>> { Data = null };
            }
        ...
        {
            var balanceResult = await this._mediator.SendAsync(new GetAccountBalancesRequest
            {
        ...
        {
            var transferAllowanceResult = await this._mediator.SendAsync(new GetTransferAllowanceQuery
            {
                AccountId = accountId
            });
